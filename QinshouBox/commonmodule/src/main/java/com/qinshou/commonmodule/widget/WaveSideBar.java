...
public class WaveSideBar extends View {
    ...
    private boolean mWave = true;   //是否波浪状
    private RectF mSelectIndexItemWindowRectF;
    private Rect mTextBounds;
    private Paint mSelectIndexItemWindowPaint;
    private boolean mShowSelectIndexItemWindow;
    ...
    public WaveSideBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mWave = typedArray.getBoolean(R.styleable.WaveSideBar_sidebar_wave, true);
        mShowSelectIndexItemWindow = typedArray.getBoolean(R.styleable.WaveSideBar_sidebar_show_select_index_item_window, true);
        typedArray.recycle();
        ...
        initPaint();
        mSelectIndexItemWindowRectF = new RectF();
        mTextBounds = new Rect();
        mSelectIndexItemWindowPaint = new Paint();
        mSelectIndexItemWindowPaint.setAntiAlias(true);
        mSelectIndexItemWindowPaint.setTextSize(120);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mStartTouchingArea.set(areaLeft,
                areaTop,
                areaRight,
                areaBottom);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, mIndexItemsLength = mIndexItems.length; i < mIndexItemsLength; i++) {
            if (TextUtils.isEmpty(mIndexItems[i])) {
                continue;
            }
        ...
        mPaint.setAlpha(255);
        mPaint.setTextSize(mTextSize);
        if (mShowSelectIndexItemWindow && mStartTouching) {
            mSelectIndexItemWindowPaint.setColor(Color.parseColor("#33000000"));
            mSelectIndexItemWindowRectF.set(mDisplayMetrics.widthPixels / 2 - 150
                    , mDisplayMetrics.heightPixels / 2 - 150
                    , mDisplayMetrics.widthPixels / 2 + 150
                    , mDisplayMetrics.heightPixels / 2 + 150);
            canvas.drawRoundRect(mSelectIndexItemWindowRectF
                    , 20
                    , 20
                    , mSelectIndexItemWindowPaint);
            mSelectIndexItemWindowPaint.setColor(Color.parseColor("#FFFFFFFF"));
            mSelectIndexItemWindowPaint.getTextBounds(mIndexItems[mCurrentIndex]
                    , 0
                    , mIndexItems[mCurrentIndex].length()
                    , mTextBounds);
            canvas.drawText(mIndexItems[mCurrentIndex]
                    , mDisplayMetrics.widthPixels / 2 - mTextBounds.width() / 2
                    , mDisplayMetrics.heightPixels / 2 + mTextBounds.height() / 2
                    , mSelectIndexItemWindowPaint);
        }
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (mStartTouchingArea.contains(eventX, eventY)) {
                    mStartTouching = true;
                    if (!mLazyRespond && onSelectIndexItemListener != null && !TextUtils.isEmpty(mIndexItems[mCurrentIndex])) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mStartTouching && !mLazyRespond && onSelectIndexItemListener != null && !TextUtils.isEmpty(mIndexItems[mCurrentIndex])) {
                    onSelectIndexItemListener.onSelectIndexItem(mIndexItems[mCurrentIndex]);
                }
                invalidate();
                return true;
...
//                }
                if (onSelectIndexItemListener != null) {
                    onSelectIndexItemListener.onCancelSelectIndexItem();
                }
                mCurrentIndex = -1;
    ...
    public void setIndexItems(List<String> indexItems) {
        mIndexItems = indexItems.toArray(mIndexItems);
        setIndexItems(Arrays.copyOf(mIndexItems, indexItems.size()));
        requestLayout();
    }
    ...
    public static abstract class OnSelectIndexItemListener {
        public abstract void onSelectIndexItem(String index);

        public abstract void onCancelSelectIndexItem();
    }
}