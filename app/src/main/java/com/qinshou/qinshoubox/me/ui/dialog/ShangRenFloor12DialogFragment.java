...
public class ShangRenFloor12DialogFragment extends DialogFragment {
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.dialog_shang_ren_floor_12, null);
                ...
                switch (radioGroup.getCheckedRadioButtonId()) {
                    case R.id.radio_button_1:
                        if (WarriorBean.getInstance().getYellowKeyCount() > 0) {
                            ...
                            WarriorBean.getInstance().setMoney(WarriorBean.getInstance().getMoney() + 7);
                            RxBus.getInstance().post(WarriorBean.getInstance());
                        } else {
                            RxBus.getInstance().post("黄钥匙数量不足");
                        }
                        break;
                    case R.id.radio_button_2:
                        if (WarriorBean.getInstance().getBlueKeyCount() > 0) {
                            ...
                            WarriorBean.getInstance().setMoney(WarriorBean.getInstance().getMoney() + 35);
                            RxBus.getInstance().post(WarriorBean.getInstance());
                        } else {
                            RxBus.getInstance().post("蓝钥匙数量不足");
                        }
                        break;
                    case R.id.radio_button_3:
                        if (WarriorBean.getInstance().getMoney() > 0) {
                            WarriorBean.getInstance().setRedKeyCount(WarriorBean.getInstance().getRedKeyCount() - 1);
                            WarriorBean.getInstance().setMoney(WarriorBean.getInstance().getMoney() + 70);
                            RxBus.getInstance().post(WarriorBean.getInstance());
                        } else {
                            RxBus.getInstance().post("红钥匙数量不足");
                        }
                        break;
