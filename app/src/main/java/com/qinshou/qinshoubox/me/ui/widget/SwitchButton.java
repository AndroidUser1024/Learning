...
public class SwitchButton extends View {
     ...
     * 开关未选中状态,即关闭状态时的背景颜色
     */
    private int mColorUnchecked = 0xFFCCCCCC;
    /**
     * 开关选中状态,即打开状态时的背景颜色
     */
    private int mColorChecked = 0xFF6495ED;
    /**
     * 开关指示器按钮的颜色
     */
...
//        mPaint.setStrokeWidth((float) getMeasuredWidth() / 40);
        ...
        if (isChecked()) {
            ...
            mPaint.setColor(getCurrentColor(mColorGradientFactor, mColorUnchecked, mColorChecked));
        } else {
            // 未选中状态时,背景颜色由选中状态的背景颜色逐渐过渡到未选中状态的背景颜色
            mPaint.setColor(getCurrentColor(mColorGradientFactor, mColorChecked, mColorUnchecked));
        }
        // 设置背景的矩形范围
        ...
        float radius = mButtonRadius != 0
                ? mButtonRadius
                : (float) getMeasuredHeight() / 2 - mButtonMargin;
        float x;
        float y;
...
//            x = radius + mPaint.getStrokeWidth() + mPaint.getStrokeWidth();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_UP) {
            ...
            if (mOnCheckedChangeListener != null) {
                mOnCheckedChangeListener.onCheckedChanged(checked, true);
            }
        }
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        ...
        mColorUnchecked = mTypedArray.getColor(R.styleable.SwitchButton_colorUnchecked, 0xFFCCCCCC);
        mColorChecked = mTypedArray.getColor(R.styleable.SwitchButton_colorChecked, 0xFF6495ED);
        mButtonColor = mTypedArray.getColor(R.styleable.SwitchButton_buttonColor, 0xFFFFFFFF);
        mButtonRadius = mTypedArray.getDimensionPixelSize(R.styleable.SwitchButton_buttonRadius, 0);
        mButtonMargin = mTypedArray.getDimensionPixelSize(R.styleable.SwitchButton_buttonMargin, (int) (mPaint.getStrokeWidth() * 4));
        mTypedArray.recycle();
    }
    ...
    public void setColorUnchecked(int colorUnchecked) {
        mColorUnchecked = colorUnchecked;
    }
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mOnCheckedChangeListener != null) {
            mOnCheckedChangeListener.onCheckedChanged(checked, false);
        }
    }
