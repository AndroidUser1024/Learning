...
package com.qinshou.qrcodemodule.camera;
...
public final class CameraManager {
    ...
    public CameraManager(Context context, ConfigManager config) {
        this.context = context;
        this.mCameraConfigurationManager = new CameraConfigurationManager(context);
        previewCallback = new PreviewCallback(mCameraConfigurationManager);
        this.mConfigManager = config;
    }
...
//    }
    ...
    public synchronized void openDriver(SurfaceHolder holder)
        ...
        if (!initialized) {
            initialized = true;
            mCameraConfigurationManager.initFromCameraParameters(theCamera);
        ...
        try {
            mCameraConfigurationManager.setDesiredCameraParameters(theCamera);
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data,
        ...
        if (mConfigManager != null && mConfigManager.isFullScreenScan()) {
            return new PlanarYUVLuminanceSource(data, width, height, 0,
                    0, width, height, false);
        } else {
            int actionbarHeight = dp2px(context, 44f);
