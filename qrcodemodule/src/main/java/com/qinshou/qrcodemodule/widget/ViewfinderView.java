package com.qinshou.qrcodemodule.widget;
...
public final class ViewfinderView extends View {
    ...
    private Bitmap scanLineBitmap;//扫描线的图片,优先选择
    ...
    public void setConfigManager(ConfigManager configManager) {
        ...
        reactColor = mConfigManager.getReactColor();
        frameLineColor = mConfigManager.getFrameLineColor();
        scanLineColor = mConfigManager.getScanLineColor();
        scanLineBitmap = mConfigManager.getScanLineBitmap();
        initPaint();
    }
    ...
    public ViewfinderView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        resultColor = Color.parseColor("#FF000000");
        resultPointColor = Color.parseColor("#FFFFBD21");
    ...
    private void initPaint() {
        ...
        if (frameLineColor != -1) {
            ...
            frameLinePaint.setColor(mConfigManager.getFrameLineColor());
    ...
    private void initAnimator() {
        if (valueAnimator == null) {
            if (scanLineBitmap != null) {
                valueAnimator = ValueAnimator.ofInt(frame.top, frame.bottom - scanLineBitmap.getHeight());
            } else {
    ...
    private void drawScanLight(Canvas canvas, Rect frame) {
        //设置了扫描线图片优先画图片
        if (scanLineBitmap != null) {
            mDst.set(frame.left, scanLineTop, frame.right, scanLineTop + scanLineBitmap.getHeight());
            canvas.drawBitmap(scanLineBitmap, null, mDst, scanLinePaint);
        } else {
